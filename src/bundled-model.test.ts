...
describe('bundled character model', () => {
  it('has three ordered parts driven by three headYaw key poses', () => {
    const model = validateModel(characterData)
    expect(model.parts.map((part) => part.id)).toEqual(['face', 'eyes', 'mouth'])
  ...
  it('tilts every part while keeping the waist and tail fixed', () => {
    const model = validateModel(characterData)
    const runtime = new MabatakiRuntime(model)
