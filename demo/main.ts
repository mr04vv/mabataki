...
import type { Binding, Keyframe, MabatakiModel, ParameterDef, Part } from '../src/index'
...
function rebuildScene(): void {
  ...
  app.renderer.resize(Math.max(...widths) + MARGIN * 2, Math.max(...heights) + MARGIN * 2)
...
app.ticker.add((ticker) => {
  if (faceTracker !== null && cameraVideoEl.currentTime !== lastVideoTime) {
    ...
    const values = faceTracker.sample(performance.now())
    if (values !== null) {
